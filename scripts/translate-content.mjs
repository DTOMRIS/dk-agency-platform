 ...
 *   --mock             Return deterministic mock translations, no DeepSeek calls
...
const MOCK_TRANSLATE = args.includes('--mock');
...
if (!DEEPSEEK_API_KEY && !DRY_RUN && !MOCK_TRANSLATE) { console.error('[FATAL] DEEPSEEK_API_KEY not set (use --dry-run or --mock for testing)'); process.exit(1); }
...
async function translateText(text, targetLang) {
  ...
  if (MOCK_TRANSLATE) return `[${targetLang}] ${text}`;
...
async function translateMissingField(row, sourceCol, targetCol, locale, label) {
  if (!row[sourceCol]) {
    stats.skipped++;
    return null;
  }
  ...
  if (DRY_RUN) {
    console.log(`    DRY-RUN: would translate ${label} (${String(row[sourceCol]).length} chars)`);
    stats.skipped++;
    return null;
  }

  const translated = await rateLimitedTranslate(row[sourceCol], locale);
  stats.inputTokensEst += estimateTokens(row[sourceCol]);
...
// Legacy row-level processors are kept for diff context while V2 processors below are active.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function processBlogPosts() {
  console.log('\n═══ blog_posts ═══');
  const rows = await sql`SELECT id, slug, title_az, summary_az, content_az, title_ru, summary_ru, content_ru, title_en, summary_en, content_en, title_tr, summary_tr, content_tr FROM blog_posts LIMIT ${ROW_LIMIT}`;
...
// ─── Main ─────────────────────────────────────────────────
async function applyFieldUpdates(tableName, id, updates) {
  const updateEntries = Object.entries(updates);
  if (updateEntries.length === 0) return false;

  await sql.query(
    `UPDATE ${tableName} SET ${updateEntries.map(([col], idx) => `${col} = $${idx + 1}`).join(', ')} WHERE id = $${updateEntries.length + 1}`,
    [...updateEntries.map(([, value]) => value), id],
  );
  return true;
}

async function processBlogPostsV2() {
  console.log('\n=== blog_posts ===');
  const rows = await sql`SELECT id, slug, title_az, summary_az, content_az, title_ru, summary_ru, content_ru, title_en, summary_en, content_en, title_tr, summary_tr, content_tr FROM blog_posts LIMIT ${ROW_LIMIT}`;
  console.log(`  Found ${rows.length} rows`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const locale of TARGET_LOCALES) {
      ...
      try {
        const updates = {};
        const titleTranslated = await translateMissingField(row, 'title_az', titleCol, locale, 'title');
        const summaryTranslated = await translateMissingField(row, 'summary_az', summaryCol, locale, 'summary');
        const contentTranslated = await translateMissingField(row, 'content_az', contentCol, locale, 'content');

        if (titleTranslated !== null) updates[titleCol] = titleTranslated;
        if (summaryTranslated !== null) updates[summaryCol] = summaryTranslated;
        if (contentTranslated !== null) updates[contentCol] = contentTranslated;

        if (await applyFieldUpdates('blog_posts', row.id, updates)) {
          stats.translated++;
...
async function processNewsArticlesV2() {
  console.log('\n=== news_articles ===');
  const rows = await sql`SELECT id, slug, title_az, summary_az, title_ru, summary_ru, title_en, summary_en, title_tr, summary_tr FROM news_articles WHERE title_az IS NOT NULL AND title_az != '' LIMIT ${ROW_LIMIT}`;
  console.log(`  Found ${rows.length} rows with title_az`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const locale of TARGET_LOCALES) {
      ...
      try {
        ...
        if (await applyFieldUpdates('news_articles', row.id, updates)) {
          stats.translated++;
          console.log(`    OK: ${Object.keys(updates).join(', ')}`);
        }
      } catch (err) {
...
async function processListingsV2() {
  console.log('\n=== listings ===');
  const rows = await sql`SELECT id, tracking_code, title, description, title_az, description_az, title_ru, description_ru, title_en, description_en, title_tr, description_tr FROM listings LIMIT ${ROW_LIMIT}`;
  console.log(`  Found ${rows.length} rows`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const locale of TARGET_LOCALES) {
      const titleCol = `title_${locale}`;
      const descCol = `description_${locale}`;
      const translationRow = { ...row, sourceTitle, sourceDesc };

      console.log(`  [${i + 1}/${rows.length}] listing #${row.id} (${row.tracking_code}) -> ${locale}`);

      try {
        const updates = {};
        const titleTranslated = await translateMissingField(translationRow, 'sourceTitle', titleCol, locale, 'title');
        const descTranslated = await translateMissingField(translationRow, 'sourceDesc', descCol, locale, 'description');

        if (titleTranslated !== null) updates[titleCol] = titleTranslated;
        if (descTranslated !== null) updates[descCol] = descTranslated;

        if (await applyFieldUpdates('listings', row.id, updates)) {
          stats.translated++;
          console.log(`    OK: ${Object.keys(updates).join(', ')}`);
        }
      } catch (err) {
...
const TABLE_MAP = {
  blog_posts: processBlogPostsV2,
  news_articles: processNewsArticlesV2,
  listings: processListingsV2,
...
for (const processor of Object.values(tablesToProcess)) {
  await processor();
}
