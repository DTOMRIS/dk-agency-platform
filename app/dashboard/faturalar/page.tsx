...
function sourceLabel(s: string, t: ReturnType<typeof useTranslations>) {
  const map: Record<string, { label: string; icon: string }> = {
    ocr_camera: { label: t('srcCamera'), icon: '📷' },
    ocr_upload: { label: t('srcOcr'), icon: '🔍' },
    manual: { label: t('srcManual'), icon: '✏️' },
    excel: { label: t('srcExcel'), icon: '📊' },
    pdf: { label: t('srcPdf'), icon: '📄' },
  };
...
function statusBadge(s: string, t: ReturnType<typeof useTranslations>) {
  const map: Record<string, { label: string; cls: string }> = {
    draft: { label: t('statusDraft'), cls: 'bg-amber-100 text-amber-700' },
    confirmed: { label: t('statusConfirmed'), cls: 'bg-emerald-100 text-emerald-700' },
    disputed: { label: t('statusDisputed'), cls: 'bg-red-100 text-red-700' },
    archived: { label: t('statusArchived'), cls: 'bg-slate-100 text-slate-600' },
  };
...
function OcrUploadModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: (row: InvoiceRow) => void }) {
  ...
  const handleFile = async (f: File) => {
    ...
    if (f.size > 1 * 1024 * 1024) {
      ...
      try {
        ...
        setCompressionInfo(`${(compressed.originalSize / 1024 / 1024).toFixed(1)}MB → ${(compressed.compressedSize / 1024 / 1024).toFixed(1)}MB (${compressed.reductionPercent}% ${t('ocrCompressionSuffix')})`);
      } catch {
  ...
  return (
    <ModalShell open={open} onClose={() => { reset(); onClose(); }} title={t('ocrModalTitle')}>
      {!file ? (
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            <span className="text-sm font-medium">{t('ocrCamera')}</span>
          </button>
          ...
          >
            ...
            <span className="text-sm">{t('ocrGallery')}</span>
          </button>
        ...
        <div className="flex flex-col gap-4">
          ...
          {compressing && (
            <div className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">
              <Loader2 className="h-4 w-4 animate-spin" /> {t('ocrCompressing')}
            </div>
          )}
          ...
          {compressionInfo && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-700">
              {t('ocrCompressionLabel')} {compressionInfo}
            </div>
          )}
          ...
          {result && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
              <div className="mb-1 flex items-center gap-1 text-sm font-medium text-emerald-700">
                <Check className="h-4 w-4" /> {t('ocrSuccess')}
              </div>
          ...
          <div className="flex gap-2">
            ...
            {!result && (
              ...
              >
                {uploading ? <><Loader2 className="h-4 w-4 animate-spin" /> {t('ocrReading')}</> : t('ocrReadBtn')}
              </button>
            )}
            {result && (
              <button
...
function ManualEntryModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: (row: InvoiceRow) => void }) {
  const t = useTranslations('dashboardFaturalar');
  ...
  const handleSave = async () => {
    if (!supplier.trim()) { setFeedback(t('manualErrSupplier')); return; }
    const validItems = items.filter((it) => it.name.trim());
    if (validItems.length === 0) { setFeedback(t('manualErrItems')); return; }
  ...
  return (
    <ModalShell open={open} onClose={() => { reset(); onClose(); }} title={t('manualModalTitle')}>
      <div className="flex flex-col gap-4">
        {/* Supplier info */}
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="mb-1 block text-xs font-medium text-slate-500">{t('manualSupplierLabel')}</label>
            <input value={supplier} onChange={(e) => setSupplier(e.target.value)} placeholder={t('manualSupplierPlaceholder')} className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#E11D48] focus:ring-1 focus:ring-[#E11D48]" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">{t('manualVoenLabel')}</label>
            <input value={voen} onChange={(e) => setVoen(e.target.value)} placeholder={t('manualVoenPlaceholder')} maxLength={10} className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#E11D48]" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">{t('manualInvoiceNoLabel')}</label>
          ...
          <div className="col-span-2">
            <label className="mb-1 block text-xs font-medium text-slate-500">{t('manualDateLabel')}</label>
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            <label className="text-xs font-medium text-slate-500">{t('manualItemsLabel')}</label>
          ...
          <div className="flex flex-col gap-2">
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-1.5 rounded-xl border border-slate-100 bg-slate-50 p-2">
                <input value={item.name} onChange={(e) => updateItem(idx, 'name', e.target.value)} placeholder={t('manualItemNamePlaceholder')} className="h-9 min-w-0 flex-[3] rounded-lg border border-slate-200 bg-white px-2 text-sm outline-none focus:border-[#E11D48]" />
                <input value={item.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} placeholder={t('manualItemQtyPlaceholder')} type="number" step="0.1" className="h-9 w-16 rounded-lg border border-slate-200 bg-white px-2 text-sm outline-none focus:border-[#E11D48]" />
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          <span className="text-sm font-medium text-slate-600">{t('manualTotalLabel')}</span>
        ...
        >
          {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> {t('manualSaving')}</> : <><Check className="h-4 w-4" /> {t('manualSave')}</>}
        </button>
      </div>
    </ModalShell>
  );
...
function ImportModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleExcel = async (f: File) => {
    ...
    try {
      ...
      setParseSummary(`${result.successRows} ${t('importRowsFound')} ${result.failedRows} ${t('importErrors')} · "${f.name}"`);
    } catch (err) {
    ...
    try {
      ...
      if (!json.success) {
        if (json.type === 'scanned') {
          setParseSummary(`"${f.name}" ${t('importScannedPdf')}`);
        } else {
      ...
      setParseSummary(`${json.successRows ?? rows.length} ${t('importRowsFound')} ${json.failedRows ?? 0} ${t('importErrors')} · "${f.name}" (${json.pageCount ?? 1} səhifə)`);
    } catch (err) {
      setParseSummary(`Xəta: ${String(err)}`);
    }
    setParsing(false);
  ...
  return (
    <ModalShell open={open} onClose={() => { reset(); onClose(); }} title={t('importModalTitle')}>
      <div className="flex flex-col gap-3">
        {!parseSummary && !parsing && (
          <>
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-emerald-800">{t('importExcelLabel')}</div>
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-blue-800">{t('importPdfLabel')}</div>
                <div className="text-xs text-blue-600">{t('importPdfHint')}</div>
              </div>
            </button>
        ...
        {parsing && (
          <div className="flex items-center gap-2 py-8 justify-center text-slate-500">
            <Loader2 className="h-5 w-5 animate-spin" /> {importSource === 'pdf' ? t('importParsingPdf') : t('importParsingExcel')}
          </div>
        )}
        ...
        {parseErrors.length > 0 && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
            <div className="mb-1 text-xs font-medium text-amber-700">{t('importErrorsLabel')}</div>
            {parseErrors.map((e, i) => (
              <div key={i} className="text-xs text-amber-600">{t('importRowLabel')} {e.row}: {e.field} — {e.error}</div>
            ))}
          </div>
        )}
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="px-2 py-1.5 font-medium text-slate-500">{t('importPreviewSupplier')}</th>
                    <th className="px-2 py-1.5 font-medium text-slate-500">{t('importPreviewProduct')}</th>
                    <th className="px-2 py-1.5 font-medium text-slate-500">{t('importPreviewQty')}</th>
                    <th className="px-2 py-1.5 font-medium text-slate-500">{t('importPreviewPrice')}</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {imported && (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">
            <Check className="mr-1 inline h-4 w-4" /> {parsedRows.length} {t('importSuccess')}
          </div>
        )}
        ...
        <div className="flex gap-2">
          {parsedRows.length > 0 && !imported && (
            ...
            >
              <Check className="h-4 w-4" /> {parsedRows.length} {t('importBtn')}
            </button>
          )}
          <button
          ...
          >
            {imported ? t('importClose') : parseSummary ? t('importReselect') : t('importCancel')}
          </button>
        </div>
      </div>
    </ModalShell>
...
export default function DashboardFaturalarPage() {
  ...
  const SOURCE_FILTERS = [
    ...
    { key: 'ocr_camera', label: `📷 ${t('srcCamera')}` },
    { key: 'ocr_upload', label: `🔍 ${t('srcOcr')}` },
    { key: 'manual', label: `✏️ ${t('srcManual')}` },
    { key: 'excel', label: `📊 ${t('srcExcel')}` },
    { key: 'pdf', label: `📄 ${t('srcPdf')}` },
  ] as const;
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setModal('ocr')} className="inline-flex items-center gap-2 rounded-xl bg-[#E11D48] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#BE123C] active:scale-[0.98]">
            ...
            <span className="sm:hidden">{t('btnCameraShort')}</span>
          </button>
          <button onClick={() => setModal('manual')} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 active:scale-[0.98]">
            ...
            <span className="hidden sm:inline">{t('btnManual')}</span>
            <span className="sm:hidden">{t('btnManualShort')}</span>
          </button>
      ...
      {stats && (
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="text-xs font-medium text-slate-500">{t('statTotal')}</div>
            <div className="mt-1 text-2xl font-bold text-slate-900">{stats.totalCount}</div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="text-xs font-medium text-slate-500">{t('statAmount')}</div>
            <div className="mt-1 text-2xl font-bold text-slate-900">{formatMoney(stats.totalAmount)}</div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="text-xs font-medium text-slate-500">{t('statAvg')}</div>
      ...
      {selected.size > 0 && (
        <div className="mb-3 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5">
          <span className="text-sm font-medium text-amber-800">{selected.size} {t('selectedCount')}</span>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="px-4 py-3 font-medium text-slate-500">{t('thSupplier')}</th>
              <th className="px-4 py-3 font-medium text-slate-500">{t('thDate')}</th>
              <th className="px-4 py-3 font-medium text-slate-500 text-right">{t('thTotal')}</th>
              <th className="px-4 py-3 font-medium text-slate-500">{t('thSource')}</th>
              <th className="px-4 py-3 font-medium text-slate-500">{t('thConfidence')}</th>
              <th className="px-4 py-3 font-medium text-slate-500">{t('thStatus')}</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm text-slate-500">{total} {t('paginationOf')} {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, total)}</div>
