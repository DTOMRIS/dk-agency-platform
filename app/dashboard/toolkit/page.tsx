...
export default function DashboardToolkitPage() {
  const t = useTranslations('dashboardToolkit');
  ...
  const editFields: [string, string][] = [
    ['titleAz', t('fieldTitleAz')],
    ['titleTr', t('fieldTitleTr')],
    ['titleEn', t('fieldTitleEn')],
    ['descriptionAz', t('fieldDescAz')],
    ['descriptionTr', t('fieldDescTr')],
    ['descriptionEn', t('fieldDescEn')],
    ['icon', t('fieldIcon')],
            ...
            <div key={item.id} className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => moveItem(index, -1)} className="rounded-full border border-slate-200 px-4 py-2 text-xs font-bold text-slate-700">{t('moveUp')}</button>
                  <button type="button" onClick={() => moveItem(index, 1)} className="rounded-full border border-slate-200 px-4 py-2 text-xs font-bold text-slate-700">{t('moveDown')}</button>
                  <button type="button" onClick={() => setEditingId(editingId === item.id ? null : item.id)} className="rounded-full bg-[var(--dk-red)] px-4 py-2 text-xs font-bold text-white">{t('edit')}</button>
                </div>
              </div>
