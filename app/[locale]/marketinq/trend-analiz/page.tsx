...
export default async function TrendAnalizRoute({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/trend-analiz'));
  }

  const access = await checkToolAccess(auth.userId, 'trend-analiz', auth.role);
  const t = await getTranslations('marketinq.trendAnaliz');
