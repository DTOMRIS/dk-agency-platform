...
export default async function ReklamRoiPage({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/reklam-roi'));
  }

  const access = await checkToolAccess(auth.userId, 'reklam-roi', auth.role);
  const t = await getTranslations('marketinq.reklamRoi');
