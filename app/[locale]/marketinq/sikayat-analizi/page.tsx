...
export default async function ComplaintAnalysisPage({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/sikayat-analizi'));
  }

  const access = await checkToolAccess(auth.userId, 'sikayet-analitigi', auth.role);
  const t = await getTranslations('marketinq.complaintAnalysis');
