...
import CustomerPersona from '@/components/marketinq/CustomerPersona';
...
export default async function CustomerPersonaPage({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/musteri-persona'));
  }

  const access = await checkToolAccess(auth.userId, 'musteri-persona', auth.role);
  const t = await getTranslations('marketinq.customerPersona');
