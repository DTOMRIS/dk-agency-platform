...
export default async function SosialMetrikRoute({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/sosial-metrik'));
  }

  const access = await checkToolAccess(auth.userId, 'sosial-metrik', auth.role);
  const t = await getTranslations('marketinq.sosialMetrik');
