...
export default async function SezonAnalitikasiPage({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const auth = await getAuthFromCookie();

  if (!auth) {
    redirect(withLocalePrefix(locale, '/auth/login?next=/marketinq/sezon-analitikasi'));
  }

  const access = await checkToolAccess(auth.userId, 'sezon-analitikasi', auth.role);
  const t = await getTranslations('marketinq.sezonAnalitikasi');

  if (!access.allowed) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
        <div className="rounded-xl border border-purple-200 bg-purple-50 p-6 text-center">
          <div className="mx-auto mb-4 inline-flex rounded-full bg-white px-3 py-1 text-xs font-bold uppercase text-purple-700">
            {t('tier')}
          </div>
