  ...
  const inputSection = (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={resetChecklist} className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-orange-600"><RotateCcw size={13} /> {t('resetBtn')}</button>
      </div>
              ...
              <button onClick={() => togglePhase(phase.key)} className="flex w-full items-center justify-between px-4 py-4 text-left">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-semibold text-slate-400">{stat.done}/{stat.total} {t('sectionCompleted')}</p>
                  </div>
                </div>
  ...
  const resultSection = (
    <div className="space-y-4">
      <div className="rounded-xl bg-white p-4 ring-1 ring-slate-200/60">
        ...
        <div className="mt-1 text-xs text-slate-400">{progress}% {t('progressCompleted')}</div>
      </div>

      {/* Phase breakdown */}
      <div className="space-y-1.5">
        {phaseProgress.map((p) => (<div key={p.key} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2"><span className="text-xs text-slate-600">{phases.find((ph) => ph.key === p.key)?.title}</span><span className={`text-xs font-bold ${p.done === p.total ? 'text-emerald-600' : 'text-slate-400'}`}>{p.done}/{p.total}</span></div>))}
      </div>
