...
## L-054: `docs/STATE.md`-ni PR-da dəyişmə — main-də bot onu özü yazır, hər dəfə konflikt yaranır
...
## L-055: Səlahiyyəti brauzerin dediyinə görə vermə — yalnız imzalı token
- **Səhv:** Login səhifəsi `plan: isAdmin ? 'admin' : 'member'`-i brauzerdə təyin edib `POST /api/member/session`-a göndərirdi; route bədəni yoxlamadan imzasız base64 cookie-yə (`dk_member_session`) yazırdı; 26 API route səlahiyyəti bu cookie-dən oxuyurdu. İstənilən şəxs bir sorğu ilə admin olurdu (TASK-0457, lokal dev-də təsdiqləndi). Əlavə: `app/dashboard/layout.tsx` yalnız JWT-nin **varlığını** yoxlayırdı — qeydiyyatdan keçən üzv lead səhifələrini (müştəri əlaqə məlumatı) aça bilirdi.
- **Kök səbəb:** İki paralel sessiya sistemi (TD-004): imzalı JWT (`dk_auth_token`, httpOnly) və imzasız «member» cookie. İkincisi UI sinxronu üçün yaradılıb, sonra səlahiyyət mənbəyinə çevrilib. «Giriş var» ≠ «admin» fərqi layout-da itib.
- **Qayda:** `loggedIn`/`plan`/`role` yalnız server tərəfindən imzalanmış tokendən oxunur; brauzerdən gələn bədən və imzasız cookie yalnız göstərmə məlumatıdır (ad). Yeni dashboard səhifəsi DB-ni birbaşa oxuyursa ilk sətir `await requireAdminPage()`; API — `requireApiAdmin()`. Təhlükəsizlik testi əvvəl köhnə kodla işə salınır (açığı tutduğunu sübut), sonra yenisi ilə.
- **Nəticə:** `getServerMemberSession` JWT-dən; `POST /api/member/session` bədəndən yalnız ad götürür; 7 dashboard səhifəsinə `requireAdminPage()`. `e2e/auth-session-security.spec.ts` köhnə kodda 4/4 FAIL, yeni kodda PASS.
