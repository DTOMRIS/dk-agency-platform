...
### Security
- `TASK-0457` fix(security): **istənilən şəxs özünü admin edə bilirdi** — TASK-0455 testini yazanda tapıldı. Login səhifəsi «admin/üzv» qərarını brauzerdə verib `POST /api/member/session`-a göndərirdi; route bədəndəki `plan: 'admin'`-i yoxlamadan imzasız base64 cookie-yə (`dk_member_session`) yazırdı; 26 API route (blog, elanlar, fakturalar, ayarlar, yükləmə, audit…) səlahiyyəti bu cookie-dən oxuyurdu. Lokal dev-də saxta cookie ilə admin endpoint-i 200 qaytardı. Düzəliş: `getServerMemberSession` səlahiyyəti (`loggedIn`, `plan`) **yalnız imzalı JWT-dən** (`dk_auth_token`, httpOnly) qurur; `POST /api/member/session` bədəndən yalnız göstəriləcək adı götürür. Yolda ikinci açıq: `app/dashboard/layout.tsx` yalnız JWT-nin varlığını yoxlayırdı → qeydiyyatdan keçən istənilən üzv `kazan-leads`, `franchise-leads`, `contact-tracking` səhifələrində müştəri əlaqə məlumatını görə bilirdi. DB-ni birbaşa oxuyan 7 dashboard səhifəsinə `requireAdminPage()` (üzv → `/b2b-panel`); layout admin-only edilmədi, çünki üzv panelindən `marketinq-ocagi`-yə keçid var (o açıq qalır). PROTECTED fayllara toxunulmadı. Mövcud admin sessiyası pozulmur (JWT artıq var). **Sübut:** yeni `e2e/auth-session-security.spec.ts` (@smoke) — köhnə kodda **4/4 FAIL** (saxta cookie 200, POST `loggedIn:true` yazır, üzv lead səhifəsi 200), yeni kodda PASS; `auth-session-security` + `dashboard-smoke` + `blog-translate-job` + `blog-md-import` **24/24** (`--repeat-each=2`); `/b2b-panel`, `/ilan-ver`, `/uzvluk`, `/settings`, bloq, `/dashboard` qonaq/üzv ilə 200/307. tsc baza (35), eslint 0. L-055.

### Fixed
- `TASK-0455` fix(blog): **«🌐 Avtomatik tərcümə» düyməsi işləmirdi** — sahib: «tercüme neden işlemiyor, deep seek var». 5 kök səbəb tapıldı: (1) mövcud RU/EN/TR mətnində AZ hərfi yoxdursa «tərcümə olunub» sayılırdı → AZ mətni Markdown faylı ilə dəyişəndə düymə «RU — · EN — · TR —» göstərib heç nə etmirdi; (2) uzun mətn hər `##`-də ayrı DeepSeek çağırışına bölünüb ardıcıl göndərilirdi (nümunədə 15 çağırış) → 1–3 dəqiqəlik sorğu proxy tərəfindən kəsilirdi; (3) bölmə ``` blokunun içindəki `#` sətrini də başlıq sayıb kod blokunu parçalayırdı; (4) yeni yazının avtomatik tərcüməsi `created.id`-yə baxırdı, amma yaradan funksiya id qaytarmır → **heç vaxt işə düşmürdü**; (5) o da yalnız «Dərc edilmiş» üçün idi, redaktor isə yeni yazını qaralama saxlayır. Düzəliş: düymə təsdiq soruşub **köhnə tərcümənin üstünə yazır** (`force`); tərcümə **arxa planda** gedir — POST dərhal 202 qaytarır (10 ms), redaktor 4 s-dən bir vəziyyəti soruşur, bitəndə yeni RU/EN/TR mətnini redaktorun state-inə yazır (əvvəl `router.refresh()` state-i yeniləmirdi → növbəti «Yadda saxla» köhnə tərcüməni geri yazardı); yadda saxlanmamış AZ dəyişikliyi varsa əvvəl saxlamağı deyir; eyni yazı üçün ikinci iş başlamır; səhifə açılanda gedən iş varsa göstərilir. Sürət: bölmələr ≤3000 simvolluq parçalara yığılır, paralel (maks. 4), sıra qorunur, ``` bloku bölünmür. Yeni yazı (qaralama da) yaradılanda avtomatik tərcümə başlayır. **Sübut:** `e2e/blog-translate-chunking.test.ts` (saxta DeepSeek) **9/9** — köhnə kodda 3 FAIL (15 çağırış, ardıcıl, kod bloku bölünür); `e2e/blog-translate-job.spec.ts` (@smoke) **6/6** (`--repeat-each=3`) — admin olmadan 403, POST 202, GET işin sonunu göstərir; `blog-md-import` 2/2. tsc baza (35), eslint 0 error. **Yoxlanmayan:** real DeepSeek və DB (sandbox-dan əlçatmaz) — redaktorda canlı sınaq sahibdədir.
