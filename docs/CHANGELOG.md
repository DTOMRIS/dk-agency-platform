...
### Added
- `TASK-0300` docs(news): **Xəbər sistemi 360° audit + sıralı icra planı** (`docs/reports/2026-06-11-news-360-audit-plan.md`). 10 paralel agent (4 kod auditi + 5 dünya/bölgə araşdırması + 1 müstəqil CTO denetleyici). Bütün 5 CEO şikayətinin kök səbəbi kod sübutu ilə təsdiqləndi: content itkisi = POST-only form + UNIQUE slug tələsi (ikinci saxlama 500 → content səssiz itir); silmə API var UI yox; edit-by-ID səhifəsi yox; 7 flag-dan 5-i consumer-siz; "Originaldan draft yarat"-da AI yoxdur. 15 əlavə kritik tapıntı: canonical→`/sektor-nebzi/` 404, admin auth bypass (Origin/Referer), `foto`/`photo` enum uyğunsuzluğu, açıq draft preview, mock RSS feed. Hüquqi çərçivə: AZ Media qanunu m.13.2 (1/3 + istinad), TR FSEK m.36/71. Köhnə 0161/0162/0163 planı LƏĞV (nömrə toqquşması + səhv sıralama) → yeni TASK-0301..0308 planı. Kod yazılmayıb — plan CEO təsdiqi gözləyir.

### Fixed
- `TASK-0301` fix(news): **SPINE — manual xəbər uçtan uca: persist + display + canonical** (PR-A). (1) Form ilk saxlamadan sonra PATCH rejiminə keçir — əvvəl hər saxlama POST idi, UNIQUE slug → ikinci saxlama 500 → yeni yazılan content SƏSSİZCƏ İTİRDİ (Ruslan Quliyev məqaləsinin kök səbəbi). Slug ilk saxlamadan sonra kilidlənir. (2) `newsType` `foto`→`photo` — köhnə dəyər bütün POST-u 400-lə öldürürdü; localStorage köhnə draft-ları miqrasiya olunur. (3) `isManual` artıq `sourceId === null`-dan çıxarılır — manual xəbərdə referans URL "Mənbədə tam xəbəri oxu" düyməsini yox, kiçik "Mənbə: hostname" sətrini göstərir (AZ media norması). (4) Canonical/hreflang/OG mövcud olmayan `/sektor-nebzi/`-dən `/haberler/`-ə — Google hər xəbər üçün 404 canonical alırdı. (5) Slug dublikat → 409 + insan dili mesaj; PATCH indi publishedAt/newsType/telegramSend/logoOverlay/externalUrl qəbul edir. Sandbox-da `next build` Google Fonts blokuna görə işləmir — canlı təsdiq deploy sonrası curl/Neon ilə.
