...
### Fixed
- `TASK-0263` fix(blog): **slug resolver + 503-safe toplu tərcümə**. (1) Detail səhifəsi: slug tapılmasa gələni `slugify` edib yenidən axtarır → kanonik slug-a 301 redirect (xam-başlıq/köhnə URL artıq 404 yox). Kök: əl ilə yaradılan postun slug-u TR başlıqdan generasiya olunmuşdu (`...isim-hakki...oduyorsun`), xam-başlıq URL uyğun gəlmirdi. `lib/utils/slug.ts` paylaşılan slugify. (2) `POST /api/blog/translate/all` artıq 15-30 dəq-lik işi bir sorğuda yox, **~40s-lik partiyalarla** işləyir (`maxDuration=60`) — Hostinger timeout/OOM/503 və app çökməsinin qarşısı. Düymə avtomatik təkrarlayır, progress göstərir, 503/HTML-i təmiz tutur (artıq "Unexpected token <" yox).
