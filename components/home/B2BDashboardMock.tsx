    ...
    <div className="relative p-2 bg-white rounded-[32px] border border-[var(--dk-border-soft)] shadow-[0_40px_80px_-15px_rgba(26,26,46,0.12)]">
      <div className="relative bg-[var(--dk-paper)] rounded-[24px] border border-[var(--dk-border-soft)] overflow-hidden aspect-[4/3] flex">
        ...
        <div className="w-[180px] bg-[var(--dk-navy)] flex flex-col h-full overflow-hidden">
          ...
          <div className="p-4 border-b border-white/5">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-[var(--dk-red)] to-[var(--dk-gold)] flex items-center justify-center text-[10px] font-bold text-white shadow-lg">
                DK
              </div>
              <div className="overflow-hidden">
                <div className="text-[10px] text-white font-bold truncate">DK Agency</div>
          ...
          <div className="p-4 space-y-6 flex-1">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between px-3 py-2 text-white/40">
                ...
                <span className="bg-[var(--dk-gold)] text-[var(--dk-navy)] text-[8px] px-1.5 rounded-full font-bold">
                  +3
                </span>
              </div>
            </div>
          ...
          <div className="p-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-3">
              ...
              <div className="text-[8px] text-white font-bold mb-1 uppercase tracking-wider">
                AI ASİSTAN
              </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-14 bg-white border-b border-[var(--dk-border-soft)] flex items-center justify-between px-6">
            <div>
              <h4 className="text-xs font-serif font-bold text-[var(--dk-navy)] mb-0.5">
                B2B Panel
              </h4>
              <div className="text-[8px] text-[var(--dk-muted)] font-semibold">
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
            ...
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                ...
                <div
                  key={i}
                  className="bg-white p-3 rounded-xl border border-[var(--dk-border-soft)] shadow-sm"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div
                      className={`w-5 h-5 rounded-md bg-[var(--dk-paper)] flex items-center justify-center ${item.cl}`}
                    >
                      {item.i}
                    </div>
                  ...
                  <div className="text-[8px] text-[var(--dk-muted)] font-bold uppercase tracking-wider">
                    {item.l}
                  </div>
                </div>
              ))}
            ...
            <div className="grid grid-cols-[1fr_120px] gap-4">
              ...
              <div className="bg-white rounded-xl border border-[var(--dk-border-soft)] overflow-hidden shadow-sm">
                <div className="px-4 py-3 border-b border-[var(--dk-border-soft)] flex justify-between items-center bg-[var(--dk-paper)]/50">
                  <span className="text-[9px] font-bold text-[var(--dk-navy)] uppercase">
                    Elanlarım
                  </span>
                ...
                <div className="divide-y divide-[var(--dk-border-soft)]">
                  {[
                    ...
                    {
                      t: 'Franchise ortağı axtarılır',
                      s: 'Yoxlamada',
                      cl: 'text-[var(--dk-gold)] bg-[color:color-mix(in_srgb,var(--dk-gold)_6%,transparent)]',
                    },
                    { t: '500k AZN Yatırım təklifi', s: 'Aktiv', cl: 'text-green-500 bg-green-50' },
                  ].map((item, i) => (
                    <div
                      key={i}
                      className="px-4 py-3 flex items-center justify-between group hover:bg-[var(--dk-paper)] transition-colors"
                    >
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <span className="text-[9px] font-bold text-[var(--dk-navy)] truncate">
                          {item.t}
                        </span>
                        <span className="text-[7px] text-[color-mix(in srgb, var(--dk-muted) 70%, white)] font-bold uppercase tracking-widest">
                          İşlətmə devri
                        </span>
                      </div>
                      <span className={`text-[7px] font-bold px-2 py-0.5 rounded-full ${item.cl}`}>
                        {item.s}
                      </span>
                    </div>
                  ))}
              ...
              <div className="space-y-4">
                <div className="text-[9px] font-bold text-[var(--dk-navy)] uppercase tracking-widest px-1">
                  Son Aktivlik
                </div>
                <div className="space-y-3">
                  {[
                    { i: <Star size={10} />, t: '2s əvvəl', c: 'bg-blue-100 text-blue-500' },
                    {
                      i: <ArrowRight size={10} />,
                      t: '5s əvvəl',
                      c: 'bg-[color:color-mix(in_srgb,var(--dk-gold)_6%,transparent)] text-[var(--dk-gold)]',
                    },
                    { i: <CheckCircle2 size={10} />, t: 'Dünən', c: 'bg-green-100 text-green-500' },
                  ].map((item, i) => (
                    <div key={i} className="flex flex-col items-center gap-1">
                      <div
                        className={`w-8 h-8 rounded-lg ${item.c} flex items-center justify-center shadow-sm`}
                      >
                        {item.i}
                      </div>
                      <span className="text-[7px] font-bold text-[color-mix(in srgb, var(--dk-muted) 70%, white)]">
                        {item.t}
                      </span>
                    </div>
                  ))}
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] text-[var(--dk-muted)] font-bold uppercase tracking-wider">
              Sistem Vəziyyəti
            </div>
